...
Диск: кожен деплой закінчується `docker image prune -af --filter
"until=168h"` — образи без контейнерів, старші за тиждень, видаляються
(звичайний `prune -f` тут не прибрав би нічого: кожен образ має тег
`:<sha12>` і «висячим» не стає). Відкат на тег у межах тижня —
миттєвий, образ ще на диску; на старіший — той самий рядок, `up -d` сам
стягне образ з GHCR, де вони лежать усі. Перевірити місце:
`docker system df` і `df -h /`.
